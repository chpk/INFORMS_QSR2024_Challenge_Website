...
class GaussianDiffusion(nn.Module):
    ...
    @torch.inference_mode()
    def p_sample_loop_r_comp(self, shape, cond = None, cond_scale = 1.):
        device = self.betas.device
        ret_img = []
        b = shape[0]
        img = torch.randn(shape, device=device)
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long), cond = cond, cond_scale = cond_scale)
            ret_img.append(unnormalize_img(img))

        return ret_img
    
    @torch.inference_mode()
    def p_sample_loop_sample(self, shape, img_input, cond = None, cond_scale = 1.):
        device = self.betas.device
    ...
    @torch.inference_mode()
    def p_sample_loop_sample_save_intermediate(self, shape, img_input, cond = None, cond_scale = 1.):
        device = self.betas.device
        print(device)
        temp_list = []
        b = shape[0]
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long), cond = cond, cond_scale = cond_scale)
            temp_list.append(unnormalize_img(img))
        return temp_list
